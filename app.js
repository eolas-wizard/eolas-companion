(() => {
  ...
  function renderHome(){
    ...
    const goals=[...missingPals.slice(0,2).map(n=>{const p=profileFor(n),loc=p.obtain?.[0]?.detail||`Search ${area}`;return `<button type="button" class="goal goal-button" data-goal-pal="${esc(n)}"><span>✦</span><div><strong>Catch ${esc(n)}</strong><small>${esc(loc)}</small><em>Open field guide ›</em></div></button>`}),...missingMarkers.slice(0,1).map(m=>`<button type="button" class="goal goal-button" data-goal-marker="${esc(m.type)}"><span>⌖</span><div><strong>${esc(m.name)}</strong><small>${esc(m.type)} · Coordinates ${esc(m.note)}</small><em>Open discoveries ›</em></div></button>`)];
  ...
  $("#exportButton").addEventListener("click",()=>{const blob=new Blob([JSON.stringify({version:"alpha-0.5.4",exportedAt:new Date().toISOString(),state},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="eolas-companion-backup.json";a.click();URL.revokeObjectURL(a.href);});
